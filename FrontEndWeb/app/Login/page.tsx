...
export default function LoginPage() {
  ...
  // ÔöÇÔöÇ LOGIN ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.status === 403) {
        setErro("Conta inativa. Entre em contato com a concession├íria.");
        setLoading(false);
        return;
      }
      ...
      setErro("N├úo foi poss├¡vel conectar ao servidor.");
    } finally {
      setLoading(false);
    }
  }

  // ÔöÇÔöÇ CADASTRO ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  async function handleCadastro(e: React.FormEvent) {
    ...
    // Valida├º├úo de CPF
    if (!validarCPF(cpf)) {
      setErroCad("CPF inv├ílido. Verifique os d├¡gitos informados.");
      return;
    }

    // Valida├º├úo de idade
    if (!dataNasc) {
      setErroCad("Informe sua data de nascimento.");
      return;
    }
    if (calcularIdade(dataNasc) < 18) {
      setErroCad("Voc├¬ precisa ter pelo menos 18 anos para se cadastrar.");
      return;
    }
    ...
    try {
      ...
      if (res.status === 409 || res.status === 400) {
        ...
        if (msg.toLowerCase().includes("cpf") || msg.toLowerCase().includes("duplicate") && msg.toLowerCase().includes("cpf")) {
          setErroCad("CPF j├í cadastrado. Utilize outro CPF ou fa├ºa login.");
        } else if (msg.toLowerCase().includes("email")) {
          setErroCad("E-mail j├í cadastrado. Tente fazer login.");
        } else if (msg.toLowerCase().includes("cpf")) {
          setErroCad("CPF inv├ílido ou j├í cadastrado.");
        } else {
          setErroCad("Dados inv├ílidos. Verifique as informa├º├Áes e tente novamente.");
        }
        return;
      }
      ...
      if (!res.ok) {
        ...
        if (msg.toLowerCase().includes("email")) {
          ...
          setErroCad("CPF j├í cadastrado ou inv├ílido.");
        } else {
      ...
      // Sucesso ÔÇö faz login autom├ítico
      ...
      if (loginRes.ok) {
        ...
        setIsCadastro(false);
        setErro("Cadastro realizado! Fa├ºa login para continuar.");
      }
    } catch {
      setErroCad("N├úo foi poss├¡vel conectar ao servidor.");
    } finally {
      setLoadingCad(false);
    }
  }
      ...
      <div className="bg-white p-8 rounded-2xl shadow-md w-96">
        ...
        <div className="flex flex-col items-center mb-6">
          ...
          <p className="text-gray-600 text-sm">
            {isCadastro ? "Crie sua conta" : "Acompanhe seu ve├¡culo em tempo real"}
          </p>
        </div>

        {/* ÔöÇÔöÇ FORMUL├üRIO DE LOGIN ÔöÇÔöÇ */}
        {!isCadastro && (
        ...
        {/* ÔöÇÔöÇ FORMUL├üRIO DE CADASTRO ÔöÇÔöÇ */}
        {isCadastro && (
        ...
        >
          {isCadastro ? "J├í tem conta? Fazer login" : "N├úo tem conta? Criar agora"}
        </p>
