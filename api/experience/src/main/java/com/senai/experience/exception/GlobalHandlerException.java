...
public class GlobalHandlerException{
    ...
    public ResponseEntity<ErrorResponse> handlerRuntimeException(RuntimeException ex){
        // Transição inválida de status → 400
        if (ex.getMessage() != null && ex.getMessage().toLowerCase().contains("transição inválida")) {
            ErrorResponse e = new ErrorResponse(400, "Requisição inválida", ex.getMessage());
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(e);
        }
        // Entidade não encontrada → 404
        if (ex.getMessage() != null && ex.getMessage().toLowerCase().contains("não encontrado")) {
            ErrorResponse e = new ErrorResponse(404, "Recurso não encontrado", ex.getMessage());
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(e);
        }
        // Demais RuntimeExceptions → 400 (Bad Request genérico)
        ErrorResponse e = new ErrorResponse(400, "Requisição inválida", ex.getMessage());
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(e);
    }
