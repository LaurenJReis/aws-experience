...
const etapas = ["Prensas","Funilaria","Pintura","Montagem","Inspeção de Qualidade"];
const descricoes = [
...
export default function SaibaMais() {
  ...
  return (
    <div className="flex min-h-screen bg-gray-100 dark:bg-gray-950">
      <Sidebar />
      <div className="flex-1 flex justify-center pb-24 md:pb-0">
        <div className="w-full max-w-5xl p-6 md:p-10">

          <div className="mb-8 text-center">
            <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">Conheça nossos processos de produção</h1>
            <p className="text-gray-600 dark:text-gray-400 mt-2">Etapa {etapa + 1} de {etapas.length}</p>
          </div>

          <div className="w-full h-[100px] md:h-[300px] rounded-2xl overflow-hidden shadow-lg mb-6">
            <iframe src={videos[etapa]} className="w-full h-full" allowFullScreen />
          </div>

          <div className="flex items-center justify-between mb-6">
            <button onClick={() => etapa > 0 && setEtapa(etapa - 1)} disabled={etapa === 0}
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-gray-400 dark:bg-gray-600 text-white shadow hover:bg-gray-300 dark:hover:bg-gray-500 disabled:opacity-40 transition">
              <ChevronLeft size={18} /> Anterior
            </button>
            <button onClick={() => etapa < etapas.length - 1 && setEtapa(etapa + 1)} disabled={etapa === etapas.length - 1}
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-red-600 text-white shadow hover:bg-red-700 disabled:opacity-40 transition">
              Próximo <ChevronRight size={18} />
            </button>
          </div>

          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100 text-center mb-4">{etapas[etapa]}</h2>
          <p className="text-base md:text-lg text-gray-700 dark:text-gray-300 leading-7 md:leading-8 text-center mb-8 px-2 md:px-10">{descricoes[etapa]}</p>
          ...
          <div>
            ...
            <div className="w-full h-3 bg-gray-300 dark:bg-gray-700 rounded-full">
              <div className="h-3 bg-red-600 rounded-full transition-all" style={{ width: `${progresso}%` }} />
            </div>
          </div>
