...
export default function VeiculosScreen({ navigation }) {
  ...
  function getProgresso(statusVeiculo) {
    const idx = STEPS.indexOf(statusVeiculo);
    if (idx === -1) return 0;
    return (idx + 1) / STEPS.length;
  }

  function renderItem({ item }) {
    const modelo = item.produto?.modelo || `Veículo #${item.id}`;
    const cor = item.produto?.cor || '';
    const ano = item.produto?.ano || '';
    const versao = item.produto?.versao || '';
    const statusFab = item.statusVeiculo;
    const progresso = getProgresso(statusFab);
    const stepAtual = STEPS.indexOf(statusFab) + 1;
...
const styles = StyleSheet.create({
  ...
  progressBg: { height: 5, backgroundColor: COLORS.ultraLight, borderRadius: 3, overflow: 'hidden' },
  progressFill: { height: 5, backgroundColor: COLORS.red, borderRadius: 3 },
  semStatusBadge: { backgroundColor: COLORS.ultraLight, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 20 },
  semStatusText: { fontSize: 11, color: COLORS.textLight, fontWeight: '600' },
});
