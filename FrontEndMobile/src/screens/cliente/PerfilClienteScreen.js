...
import {
  getPessoaFisica, updatePessoaFisica,
  getPessoaJuridica, updatePessoaJuridica,
} from '../../services/api';
...
export default function PerfilClienteScreen() {
  ...
  const fetchPerfil = useCallback(async () => {
    setLoading(true);
    try {
      if (!user?.id) { setForm(user || {}); return; }
  ...
  const avatarLetter = (form.nome || form.razaoSocial || 'C')[0]?.toUpperCase();
      ...
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        <View style={styles.avatarSection}>
          ...
          <View style={styles.roleBadge}>
            <Text style={styles.roleText}>{isPF ? 'PESSOA FÍSICA' : 'PESSOA JURÍDICA'}</Text>
          </View>
        </View>
        ...
        <Card>
          ...
          {!isPF && (
            <Input
              ...
              onChangeText={(v) => set('razaoSocial', v)}
          ...
          {editing && (
            <View style={styles.actionRow}>
              <Button
                title="Cancelar"
                variant="outline"
                onPress={() => { setEditing(false); fetchPerfil(); }}
...
const styles = StyleSheet.create({
  ...
  avatar: {
    ...
    backgroundColor: '#17A2B8', alignItems: 'center', justifyContent: 'center', marginBottom: 12,
  },
  avatarText: { color: COLORS.white, fontSize: 32, fontWeight: '700' },
  userName: { fontSize: 18, fontWeight: '700', color: COLORS.dark },
  roleBadge: {
    marginTop: 6, backgroundColor: COLORS.ultraLight,
    paddingHorizontal: 14, paddingVertical: 4, borderRadius: 20,
  },
  roleText: { fontSize: 11, fontWeight: '700', color: COLORS.mediumGray, letterSpacing: 1 },
