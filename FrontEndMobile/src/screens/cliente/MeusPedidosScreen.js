...
export default function MeusPedidosScreen({ navigation }) {
  ...
  // Recarrega quando chega notificação
  useEffect(() => {
    if (lastNotification) fetchPedidos();
  }, [lastNotification]);
  ...
  function renderItem({ item }) {
    const produto = item.itens?.[0]?.produto;
    const modelo = produto?.modelo || `Pedido #${item.id}`;
    const detalhes = [produto?.versao, produto?.cor, produto?.ano].filter(Boolean).join(' · ');
    return (
      <TouchableOpacity
        style={styles.card}
      ...
      >
        <View style={styles.cardTop}>
          ...
          <View style={styles.cardInfo}>
            ...
            <Text style={styles.cardSub}>{detalhes || `Pedido #${item.id}`}</Text>
            <Text style={styles.cardDate}>{formatDate(item.dataPedido)}</Text>
          </View>
        ...
        <View style={styles.cardBottom}>
          ...
          {item.vendedor?.nome && (
            <Text style={styles.cardVendedor}>Vendedor: {item.vendedor.nome}</Text>
          )}
        </View>
      </TouchableOpacity>
    );
...
const styles = StyleSheet.create({
  ...
  cardVendedor: { fontSize: 11, color: COLORS.textLight },
});
